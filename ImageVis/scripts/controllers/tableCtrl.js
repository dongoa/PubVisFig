...
vishope.controller('tableCtrl', ['$scope', 'pipService', 'dataService','egoVisService',
    function ($scope, pipService, dataService,egoVisService) {
        console.log("这里是tableCtrl.js");
        $scope.selection = ['2006T01','2007T01'];
        ...
        if ($scope.nodesData != undefined) {
            ...
            // }, function (promise) {
            //     alert('Table Error loading node schema!');
            // });
        }
        console.log("table-pipsevice-datachange");
        pipService.onDatasetChange($scope, function (msg) {
            console.log("tableCtrl-datachange");
            dataService.getNodeSchema().then(function (promise) {
                initTable(promise.data);
                var page = $scope.tableConfig.currentPage;
        ...
        pipService.onOverviewClicked($scope, function (msg) {
            ...
            for (var i = 0; i < $scope.nodesData.length; i++) {
                if ($scope.nodesData[i]['id'] == msg) {
                    ...
                    console.log("tableCtrl-node",node);
                    break;
                }
            }
        ...
        pipService.onAlterDoubleClicked($scope, function (msg) {
            console.log("onAlterDoubleClicked");
            var node;
            var hasData = false;
            for (var i = 0; i < $scope.nodesData.length; i++) {
                if ($scope.nodesData[i]['id'] == msg) {
